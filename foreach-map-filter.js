...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(value, i) {
        ...
        let secondChar = value.charAt(value.length - 1);
        newArr.push(firstChar + secondChar);
    })
    return newArr;
}
...
function vowelCount(str){
    ...
    let newWord = lowerCaseWord.split("")
    let vowels = ['a', 'e', 'i', 'o', 'u'];
    const vowelCountObj = {};
    
    for (let vowel of vowels) {
        vowelCountObj[vowel] = 0;
    }

    newWord.forEach(function(value) { 
        if (value.includes('a') || value.includes('e') || value.includes('i') || value.includes('o') || value.includes('u')) {
            vowelCountObj[value] ++;
        } else {
            delete vowelCountObj[value]
        }    
    })
    
    for (let vowel of vowels) {
        if (vowelCountObj[vowel] == 0) {
            delete vowelCountObj[vowel]
        }
    }
...
function extractFullName(arr){
    return arr.map(function(object, first, last) {
...
function filterByValue(arr, key) {
    const newArr = arr.filter(function(object) {
        return object[key];
    })
    return newArr;
}
